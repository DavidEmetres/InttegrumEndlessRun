...
public class TileArrayManager {
	...
	public List<GameObject> mediterraneanEnviros = new List<GameObject>();
	...
	public TileArrayManager() {
		...
		GameObject[] allTileArrays = (Resources.LoadAll (path, typeof(GameObject))).Cast<GameObject> ().ToArray ();

		for (int i = 0; i < allTileArrays.Length; i++) {
			string name = allTileArrays [i].name;
			string[] t = name.Split ('_');

			if (t [0] == "Oceanic")
				oceanicTileArrays.Add (allTileArrays [i]);
			else if (t [0] == "Continental")
				continentalTileArrays.Add (allTileArrays [i]);
			else if (t [0] == "Mediterranean")
				mediterraneanTileArrays.Add (allTileArrays [i]);
		}

		path = "Enviros/";

		GameObject[] allEnviros = (Resources.LoadAll (path, typeof(GameObject))).Cast<GameObject> ().ToArray ();

		for (int i = 0; i < allEnviros.Length; i++) {
			string name = allEnviros [i].name;
			string[] t = name.Split ('_');

			if (t [0] == "Oceanic")
				oceanicEnviros.Add (allEnviros [i]);
			else if (t [0] == "Continental")
				continentalEnviros.Add (allEnviros [i]);
			else if (t [0] == "Mediterranean")
				mediterraneanEnviros.Add (allEnviros [i]);
		}
...
//		}
	...
	public GameObject GetRandomTileArray() {
		int i;
